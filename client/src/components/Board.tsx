  ...
  return (
    <div className={styles.board}>
      {Array.from({length: 6}, (_, i) => i < guesses.length ? <Row key={i} letters={guesses[i].split('')} states={results[i].map(r => r.result)} shaking={false}/>
        : i === guesses.length ? <Row key={i} letters={currentGuess.split('').concat(Array(5 - currentGuess.length).fill(''))} states={Array(currentGuess.length).fill('tbd')} shaking={shaking}/> 
